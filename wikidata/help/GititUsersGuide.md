...
Including a page within a page
------------------------------
The substitution plugin allows you to include a page of markdown verbatim within another page.
You have to create a page in the wikidata page with extension .page, either using the website interface of creating a page by creating a link or by editing the file manually and adding it to the git. To include the markdown text from that page when writing another page, use `[pagename](!subst)`, where pagename is the filename of a page called pagename.page on the disk. The notation of the plugin is made such that it's the same as inserting a link to the page in question, just modified by adding !subst to the argument. This is particularly helpful for including pages that are automatically generated/dynamically updated, but retain a static name.
